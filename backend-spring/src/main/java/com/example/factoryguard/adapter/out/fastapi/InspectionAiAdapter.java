...
public class InspectionAiAdapter implements CallAiInspectionPort {
    ...
    private AiInspectionResult toResult(AiInspectionResponse response) {
        ...
        return AiInspectionResult.builder()
                .score(response.getData().getAnomalyScore() == null ? response.getData().getScore() : response.getData().getAnomalyScore())
                .confidence(response.getData().getConfidence())
                .modelVersionId(response.getData().getModelVersionId())
                .imagePath(response.getData().getImagePath())
                .categoryType(response.getData().getCategoryType())
                .category(response.getData().getCategory())
                .modelProfile(response.getData().getModelProfile())
                .modelName(response.getData().getModelName())
                .imageThreshold(response.getData().getImageThreshold())
                .predictedLabel(response.getData().getPredictedLabel())
                .heatmapPath(response.getData().getHeatmapPath())
                .pixelThreshold(response.getData().getPixelThreshold())
                .inferenceTime(response.getData().getInferenceTime())
