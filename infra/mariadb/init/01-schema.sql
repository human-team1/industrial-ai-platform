-- Generated baseline for Docker entrypoint (fresh volume).
-- Source: industrial-ai-platform.sql — IF NOT EXISTS for tables/indexes.
...
CREATE TABLE IF NOT EXISTS user_threshold_history (
...
CREATE TABLE IF NOT EXISTS inspection_input (
...
CREATE TABLE IF NOT EXISTS inspection_event_log (
...
CREATE TABLE IF NOT EXISTS anomaly_region (
...
CREATE TABLE IF NOT EXISTS operation_policy (
...
CREATE INDEX IF NOT EXISTS idx_inspection_run_org_user ON inspection_run(organization_id, user_id);
CREATE INDEX IF NOT EXISTS idx_inspection_run_status ON inspection_run(run_status);
CREATE INDEX IF NOT EXISTS idx_inspection_run_org_completed ON inspection_run(organization_id, completed_at);
CREATE INDEX IF NOT EXISTS idx_inspection_run_org_status_completed ON inspection_run(organization_id, run_status, completed_at);
CREATE INDEX IF NOT EXISTS idx_result_decision ON inspection_result(decision_code);
CREATE INDEX IF NOT EXISTS idx_result_inspection_decision ON inspection_result(inspection_id, decision_code, final_decision_code, created_at);
...
CREATE INDEX IF NOT EXISTS idx_model_version_available ON model_version(model_id, model_category, deploy_status, is_active, created_at);
...
CREATE INDEX IF NOT EXISTS idx_model_deployment_available ON model_deployment(organization_id, target_id, deployment_scope, deploy_status, is_active, deployed_at);
CREATE INDEX IF NOT EXISTS idx_model_deployment_version_active ON model_deployment(model_version_id, is_active, deploy_status);
...
CREATE INDEX IF NOT EXISTS idx_analysis_target_org_status ON analysis_target(organization_id, target_status);
...
CREATE INDEX IF NOT EXISTS idx_async_job_type_status_created ON async_job(job_type, job_status, created_at);
CREATE INDEX IF NOT EXISTS idx_file_storage_object ON file(storage_type, bucket_name, object_key);
CREATE INDEX IF NOT EXISTS idx_file_created_by ON file(created_by, created_at);
CREATE INDEX IF NOT EXISTS idx_inspection_input_file ON inspection_input(file_id);
CREATE INDEX IF NOT EXISTS idx_document_org_owner ON document(organization_id, owner_user_id);
CREATE INDEX IF NOT EXISTS idx_document_version_status_created ON document_version(indexing_status, created_at);
CREATE INDEX IF NOT EXISTS idx_document_index_job_status ON document_index_job(job_status, document_version_id);
CREATE INDEX IF NOT EXISTS idx_notification_user_read ON notification(user_id, is_read);
CREATE INDEX IF NOT EXISTS idx_chat_conversation_user ON chat_conversation(user_id);
CREATE INDEX IF NOT EXISTS idx_chat_conversation_user_updated ON chat_conversation(user_id, updated_at);
...
CREATE INDEX IF NOT EXISTS idx_document_equipment_type ON document(equipment_type);
CREATE INDEX IF NOT EXISTS idx_document_tag_name ON document_tag(tag_name);

-- Extra indexes from repo migrations (not in original init index list).
CREATE INDEX IF NOT EXISTS idx_notification_user_read_created ON notification(user_id, is_read, created_at);
CREATE INDEX IF NOT EXISTS idx_notification_user_type_created ON notification(user_id, notification_type, created_at);
CREATE INDEX IF NOT EXISTS idx_notification_user_dedup ON notification(user_id, dedup_key);
CREATE INDEX IF NOT EXISTS idx_model_deployment_slot_deactivate ON model_deployment (organization_id, deployment_scope, target_id, is_active, deploy_status, model_version_id);
CREATE INDEX IF NOT EXISTS idx_model_version_slot_lookup ON model_version (model_category, model_profile, model_version_id);
CREATE INDEX IF NOT EXISTS idx_document_index_job_ai_job_id ON document_index_job(ai_job_id);
