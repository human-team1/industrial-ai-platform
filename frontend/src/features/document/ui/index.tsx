...
export function indexingStatusLabel(status: IndexingStatus | string | undefined | null): string {
  ...
  if (s === 'COMPLETED') return '반영완료'
  if (s === 'FAILED') return '반영실패'
            ...
            {items.map((item) => (
              <tr key={item.documentId} className="border-b border-slate-100 last:border-0">
                ...
                <td className="px-4 py-3 space-x-2">
                  <button className="btn-secondary" onClick={() => onEdit(item.documentId)}>수정</button>
                  <button className="btn-secondary" onClick={() => onDelete(item.documentId)}>삭제</button>
                </td>
              </tr>
            ))}
          </tbody>
...
function StatusBadge({ status }: { status: IndexingStatus }) {
  const label = indexingStatusLabel(status)
  const color = status === 'PENDING' ? 'bg-slate-100 text-slate-700' : status === 'PROCESSING' ? 'bg-blue-100 text-blue-700' : status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'
